...
void position_write_callback(const swiftpro::position& msg)
{
	...
	sprintf(y, "%.2f", msg.y);
	sprintf(z, "%.2f", msg.z);
	Gcode = (std::string)"G0 X" + x + " Y" + y + " Z" + z + " F99999999" + "\r\n";
...
void arm_reset_callback(const std_msgs::String::ConstPtr &msg)
{
...
{	
	...
	ros::Subscriber sub6 = nh.subscribe("arm_reset_topic", 1, arm_reset_callback);
	...
	if (_serial.isOpen())
	{
		...
		_serial.write("M17\r\n");				// attach
		ros::Duration(0.1).sleep();				// wait 0.1s
		_serial.write("M201 X4000 Y4000 Z4000\r\n"); // M201 - Set max acceleration in units/s^2 for print moves (M201 X1000 Y1000)
		ros::Duration(0.1).sleep();				// wait 0.1s
		_serial.write("M203 X10000 Y10000 Z10000\r\n"); // M203  - Set maximum feedrate that your machine can sustain (M203 X200 Y200 Z300 E10000) in units/sec
		ros::Duration(0.1).sleep();				// wait 0.1s @TODO 
		_serial.write("M204 P2000 T2000\r\n"); // M204 - Set default acceleration: P for Printing moves, R for Retract only (no X, Y, Z) moves and T
		// 														   // for Travel (non printing) moves (ex. M204 P800 T3000 R9000) in units/sec^2
		ros::Duration(0.1).sleep();				// wait 0.1s
		_serial.write("M204 A400\r\n"); // M204 - Set default acceleration: P for Printing moves, R for Retract only (no X, Y, Z) moves and T
		// 														   // for Travel (non printing) moves (ex. M204 P800 T3000 R9000) in units/sec^2
		ros::Duration(0.1).sleep();				// wait 0.1s
		_serial.write("M205 X20 Z20\r\n");  // M205 - Set advanced settings. Current units apply:  S<print> T<travel> minimum speeds
        //     									// B<minimum segment time>  X<max xy jerk>, Z<max Z jerk>, E<max E jerk>
		ros::Duration(0.1).sleep();				// wait 0.1s
		//2.标定时这段注释,运行时保留
		ros::Duration(3.5).sleep();				// wait 3.5s
		_serial.write("M2120 V0\r\n");			// stop report position
