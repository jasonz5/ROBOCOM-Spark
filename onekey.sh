...
cal_camera3_arm(){
	...
	if [ $ROSVER = "kinetic" ]; then
		echo -e "${Info}It is kinetic." 
	  	roslaunch spark_carry_object spark_carry3_cal_cv3.launch camera_type_tel:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	elif [ $ROSVER = "indigo" ]; then
		echo -e "${Info}It is indigo." 
	  	roslaunch spark_carry_object spark_carry3_cal_cv2.launch camera_type_tel:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	elif [ $ROSVER = "melodic" ]; then
		echo -e "${Info}It is melodic." 
	  	roslaunch spark_carry_object spark_carry3_cal_cv3.launch camera_type_tel:=${CAMERATYPE} lidar_type_tel:=${LIDARTYPE}
	fi
...
————————————
  ...
  ${Green_font_prefix} 14.${Font_color_suffix} 机械臂与摄像头做第三层标定
———————————— "
check_camera_game
echo -e "
